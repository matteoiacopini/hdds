#' Creates and plots a single HDDS, upside down, from data points
#'
...
#' hdds_dw(data=DATA3, discrete=FALSE, bounds=NA, add.median=TRUE, inv.scale=20,
#'         colors.from.to=colors.from.to, add.data.points=TRUE, labels=TRUE)
#'
...
hdds_dw = function(data,
   ...
   # create HDDS object
   colBin = circleFun_dw(center=circle.center, diameter=diameter, start=0, end=1/NBINS, filled=F)
   hc = ggplot() +
   ...
   # fill-in the HDDS
   for(i in 1:(NBINS-0)){
      colBin = circleFun_dw(center=circle.center, diameter=diameter, start= 1-i/NBINS, end= 1-(i-1)/NBINS, filled=T)
      hc = hc + geom_polygon(data=colBin, aes(x,y), fill=BIN.COLORS[i])
   }

   # circle border
   colBin = circleFun_dw(center=circle.center, diameter=diameter, start=0, end=1, filled=F)
   hc = hc + geom_path(data=colBin, aes(x,y), color=colors.from.to[2], size=0.5) #, color="black")
   ...
   if (add.median){
      ...
      #       (  -cos(angle), sin(angle)   ) * rr + circle.center
      # then, change sign of y-coordinate since the half-disk (hence the angle) is specular wrt the x-axis
      # this is equivalent to add pi
      ...
      # lim_old = c(bounds[1],bounds[2])
      # lim_new = c(1.0,-1.0)
      # r_outer = diameter/2+0.04
      # r_inner = diameter/2-0.02
      # # coordinates on outer cirrcle (ending point)
      # xend = lim_new[1] + ( (lim_new[2]-lim_new[1]) / (lim_old[2]-lim_old[1]) )*(med - lim_old[1])
      # yend = -sqrt( r_outer^2 - xend^2 )
      # # coordinates on inner cirrcle (starting point)
      # midX = (xend) / 2
      # midY = (yend) / 2
      # len = sqrt(midX*midX + midY*midY)
      # x0 = r_inner * midX / len
      # y0 = r_inner * midY / len
      # hc = hc + geom_segment(aes(x = x0, y = y0, xend = xend, yend = yend), color="black", linetype="solid", size=1.10)
   }
   ...
   if(add.data.points){
      ...
      for(i in 1:(NBINS+1)){
         ...
         yy[i] = r_points * sin(theta)*(-1)  # get y-coordinate
      }
      xx = sort(xx)
      ...
      # r_points = diameter/2-0.04
      # lim_old = c(bounds[1],bounds[2])
      # lim_new = c(r_points,-r_points)
      # # coordinates on outer cirrcle (ending point)
      # xend_pts = lim_new[1] + ( (lim_new[2]-lim_new[1]) / (lim_old[2]-lim_old[1]) )*(dots - lim_old[1])
      # yend_pts = -sqrt( r_points^2 - xend_pts^2 ) #+ circle.center[2]
   }

   # add LABELS
   if(labels == TRUE){
      hc = hc +
         annotate("text", x=circle.center[1]+0.56*diameter, y=circle.center[2]-0.01*diameter,
   ...
   if(legend == TRUE){
      hc = hc +
         ...
         theme(legend.direction = "horizontal", legend.position=c(0.47,0.95)) #"bottom") #c(0.98,0.5))
   }else{
      hc = hc +
         theme(axis.line=element_blank(), axis.text.x=element_blank(), axis.text.y=element_blank(),
               axis.ticks=element_blank(), axis.title.x=element_blank(), axis.title.y=element_blank(),
